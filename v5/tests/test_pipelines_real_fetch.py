...
class TestRLRealFetch:
    def test_fetch_no_token_silently_returns_empty(self, cfgs, tmp_path, monkeypatch):
        ...
        # _list_rlcs_replays() will get 401 from API; mock it
        with patch.object(pipeline.session, "get", side_effect=Exception("401")):
            paths = pipeline.fetch()
        assert paths == []
    ...
    def test_fetch_with_mocked_api(self, cfgs, tmp_path, monkeypatch):
        ...
        list_response.json.return_value = {"list": [{"id": "rep1"}, {"id": "rep2"}]}

        download_response = MagicMock()
        download_response.status_code = 200
        download_response.iter_content = lambda chunk_size: [b"FAKE_REPLAY"]

        def get_side_effect(url, *args, **kwargs):
            if "/replays/" in url and "/download" in url:
    ...
    def test_fetch_handles_rate_limit(self, cfgs, tmp_path, monkeypatch):
        ...
        list_response = MagicMock()
        list_response.status_code = 200
        list_response.json.return_value = {"list": [{"id": "rep1"}]}
        ...
        with patch.object(pipeline.session, "get", side_effect=get_side_effect), \
             patch("v5.src.cells.rocket_league.pipeline.time.sleep"):
            paths = pipeline.fetch()
        # Rate-limited download → no path returned
        assert len(paths) == 0

    def test_parse_replay_no_parser_available(self, cfgs, tmp_path):
        pipeline = RocketLeaguePipeline(config=cfgs["rocket_league"], data_root=tmp_path)
        replay = tmp_path / "test.replay"
        replay.write_bytes(b"fake")

        # Both carball and rrrocket missing → returns None
        with patch.dict("sys.modules", {"carball": None}), \
             patch("subprocess.run", side_effect=FileNotFoundError("rrrocket")):
            result = pipeline._parse_replay(replay)
        assert result is None

    def test_parse_replay_with_rrrocket_fallback(self, cfgs, tmp_path):
        pipeline = RocketLeaguePipeline(config=cfgs["rocket_league"], data_root=tmp_path)
        replay = tmp_path / "test.replay"
        replay.write_bytes(b"fake")
        ...
        mock_result.stdout = json.dumps({"properties": {"Goals": {"value": []}}})
    ...
    def test_parse_loads_existing_processed(self, cfgs, tmp_path):
        pipeline = RocketLeaguePipeline(config=cfgs["rocket_league"], data_root=tmp_path)
        replay = tmp_path / "test.replay"
        replay.write_bytes(b"fake")
        # Pre-create processed JSON
        processed = pipeline.processed_dir / "test.json"
        processed.write_text('{"id": "test", "_hits": []}')
